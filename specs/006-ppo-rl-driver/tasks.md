...
- [ ] T046 [US3] **Runs a and b of three done.** Run `ppo_car_spread_a`, `ppo_car_spread_b` and `ppo_car_spread_c`: identical configuration at the reduced budget, differing only in `--seed`. Three rows in `results/EXPERIMENTS.md`, each written in its own session
