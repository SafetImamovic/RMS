...
- [X] T057 Write the **measured** results back into `DESIGN.md` in a `docs:` commit: the throughput actually achieved, the spread actually measured, and the lap rates actually reached against the criterion section 5 states. Decided values went in at T001; only what had to be measured lands here
- [X] T058 Add the M3 recipe to `README.md` in this feature, per Principle VI: the trainer command from the repository root, the single `Ctrl+C`, the curve export, and the evaluation sweep. The recipe changes in the same feature that makes it true
...
- [X] T060 Run the full `pytest` suite in `.venv` and `.venv-bc`, and record the counts against T004's baseline. Every new test is accounted for and no existing test regressed
  ...
  - **Re-measured after T052 added nine tests: `.venv` 347 passed, 3 skipped** in 201.7 s and **`.venv-bc` 401 passed** in 208.2 s. Still no regression; the delta against the first measurement is exactly `test_rl_report.py`, and the 401 was predicted from that argument before it was run, then measured rather than left as a prediction
  - Final tally against T004's baseline: **`.venv` 323 to 347, `.venv-bc` 377 to 401, +24 in both**, all accounted for as `test_rl_curves.py` (15) plus `test_rl_report.py` (9)
...
- [X] T061 Close out the spec: mark each success criterion met or not met with the number that decides it, and record any that this feature could not reach and why
- [X] T062 Audit what is staged before the merge: no `events.out.tfevents.*`, no `checkpoint*.pt`, no intermediate `.onnx` snapshots, and no `Library/`, `dataset/` or `.venv*/`. The committed model is LFS-routed and the curves are CSV
- [X] T063 Work the merge checklist in `CONTRIBUTING.md`, confirm every new `Assets/` file has its `.meta`, and confirm each `.meta` sits in the same commit as its file
  - Checklist worked 2026-08-24. **1.** `git status` clean of `Library/`, `dataset/`, `.venv*`. **2.** Every `Assets/` file added since `e0a7d4b` carries its `.meta` in the same commit, checked by pairing each non-meta path against `<path>.meta`. **3.** 109 EditMode tests green and 347/401 pytest. **5.** All three models listed by `git lfs ls-files`. **6.** No `Co-Authored-By` and no session link in any of this feature's commits, checked across the whole range
  - **T062 found two `checkpoint.pt` files tracked, and they are not this feature's.** `results/bc/run_bc_{balanced,unbalanced}_v01/checkpoint.pt` came from feature 004 in `bfd407c` on 2026-08-08, are LFS-routed, and were a deliberate M4 decision. Left alone: removing another feature's committed artefacts is outside this merge, and the audit's intent is that *this* feature adds none
  - This feature's `.onnx` are three **final** models, not intermediates. Zero `CarDriver-<step>.onnx` snapshots from the trainer's own directories are tracked; `results/ppo_car*/` is ignored at `.gitignore:89`. The step suffix in `Assets/Models/ppo_car_spread_a-2000063.onnx` is what T038 asked for, not a snapshot
  - **One inconsistency found and deliberately not fixed here.** `CONTRIBUTING.md` still says the agent never runs `git commit`, which constitution 2.0.0 superseded on 2026-08-19 by allowing agent commits on explicit instruction. The two documents disagree and the constitution wins, but correcting a governance document is not a task this feature owns; raised for the owner instead
