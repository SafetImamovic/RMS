...
| II. Git-Flow & Atomic Commits | **DEVIATION** | Branch is `004-bc-baseline`; the constitution requires `feature/<kebab-desc>`. See Complexity Tracking |
...
|-----------|------------|-------------------------------------|
| Branch named `004-bc-baseline`, not `feature/bc-baseline` as Principle II requires | The spec-kit script generates the numbered form and the repository already carries both shapes from earlier features (`002-data-authenticity` alongside `feature/data-authenticity`) | Not rejected, just not yet done. Recommend renaming to `feature/bc-baseline` before any commit lands, or amending Principle II to accept the numbered form. Left as a deviation rather than resolved silently, because the constitution says to stop and propose rather than deviate quietly |
