...
- [ ] T006 Write `python/bc/survey.py` and run it: count recording sessions in the combined dataset via `eda.integrity.split_sessions`, report each session's row count, time range and track, and list the validation fractions actually achievable by holding out whole sessions. Write to `results/bc/session_survey.md`
  - **This is a blocking reconnaissance task, not a formality.** Research R2 chose session-level holdout to make the leak-free property parameter-free, but that choice only works if there are enough sessions to form a representative validation set. If the survey finds very few sessions, the split strategy has to change before anything is built on it, and that is far cheaper to learn now than after a training run
...
- [ ] T009 Write the **decided** M4 values into `DESIGN.md` section 6 in a `docs:` commit, before any Phase 3 code exists. Four amendments, all in one logical change:
  - **Split**: section 6.2 currently says "Split: 80/20 train/val" with no mention of leakage. Amend to session-level holdout, stating that consecutive frames are near-duplicates and that a random frame split scores the model on frames it effectively trained on. Record the achieved fraction from T006 and state that it is reported rather than forced
...
- [ ] T011 [US1] Implement `plan_split`, `write_split` and `read_split` in `python/bc/split.py`, operating on whole sessions from `eda.integrity.split_sessions`, seeded and never dividing a session. Produce the `SplitPlan` fields from `data-model.md`, with `val_fraction_actual` derived and reported rather than forced
- [ ] T012 [US1] Implement `verify_no_leak` in `python/bc/split.py`, raising and naming both sessions and the overlap when any validation session's time range meets a training session's. This is the machine-checkable form of FR-004
- [ ] T013 [US1] Add the `python -m python.bc.split --seed <n> --val-fraction <f>` command line entry, writing `results/bc/split.json`
- [ ] T014 [US1] Write `python/tests/test_bc_split.py`: the same seed produces byte-identical output across two calls and across processes; `verify_no_leak` accepts a plan built from real sessions; a hand-built plan splitting one session across both sides is rejected with the session named; an empty validation side is rejected; `val_fraction_actual` is asserted to be **reported**, with the test allowing it to differ from the target rather than requiring it to match
...
- T006 blocks T009, because the split amendment cannot honestly state an achieved fraction before the sessions are counted.
