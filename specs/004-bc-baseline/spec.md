...
- **The camera offset is jittered, not constant.** The design document states plus or minus 0.2. Measured, that constant places 40.6 percent of training targets on exactly two lattice values, creating modes the human never produced. The offset is therefore drawn per sample from 0.10 to 0.30, mean unchanged at 0.20, held identical across both balancing runs. It remains a chosen range rather than a derived one, because the true correction depends on speed and curvature that this dataset does not document.
