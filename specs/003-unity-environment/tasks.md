...
- [X] T062 [US3] Read every observation live during a human drive and check each against a situation whose correct answer is visible. Record which observation was checked against which situation (FR-029, the M2 gate)
  - **All nineteen observations verified on 2026-08-09, on seed 1004 and on `FlatGround`. Recorded in `t062-observation-check.md`.** Every one has been read away from its resting value, in both signs where it has one, and every divisor has been checked against something computed independently of it
  - Seven of the twelve situations turned out to be static, and for those the correct answer follows from the track geometry rather than from what is visible out of the window. Those were checked by computing the expected reading in Python from the track JSON and comparing against what Unity measured. **Worst disagreement across the thirteen rays: 0.000 m.** Two independent implementations agreeing to three decimals is a stronger claim than a person judging a barrier by eye, and it catches what a person cannot: a half-degree spacing error, or a sign convention that is right at 90 degrees and wrong at 15
  - `ObservationProbe` was added for the driving half: `P` prints the whole vector as one paste-able block, for the same reason `LapReport` exists. Nineteen numbers transcribed by hand off a panel updating at 50 Hz is how a wrong figure reaches a task record
  - The reading that matters most, both halves of it in one frame: **a barrier on the bumper reads 0.029 and a clear horizon reads 1.000.** FR-025 turns on those being at opposite ends of the range, and the mirrored mistake would make the safest reading identical to the most dangerous one
  - Two incidental confirmations. `ray 00 + ray 12 = 6.033 m` against a declared track width of 6.000, so the width is measurable from inside the sim. `MaxYawRateRadPerS` reads 1.865 live, matching 10.0 / 5.361, which confirms the chain from `vehicle_profile.json` through `CarController.Profile` into the observation scale
  - Repositioning the car for a static pose exposed **C16 from the opposite direction**: writing `transform.position` in play mode is silently discarded, because the Rigidbody owns the pose and overwrites it at the next physics step. `Rigidbody.position` works. The write appears to succeed either way, which is precisely what hid the spawn defect
  - **The one prediction this task made that failed is the most useful thing in it.** At full lock the yaw reading and the forward speed should be the same number, because the radius cancels. Measured: yaw pinned at ±1.000 while forward speed never passed 0.801. The car was sliding at 4.59 m/s sideways, a 30 degree slip angle, and rotating at least 25 percent faster than the no-slip bound allows. The identity holds only in steady-state no-slip cornering, and full lock at full throttle is not that
  - Consequence for M3, recorded rather than fixed: **`YawRateNorm` saturates under ordinary inputs**, so during a spin the yaw observation is a constant. That is the situation where recovery information matters most. The clamp is right and widening the divisor would hide the event rather than report it. The note exists so an agent that cannot recover from spins is not diagnosed as needing more training
  - Because every yaw reading was either near zero or clamped, the divisor was checked three other ways: `MaxYawRateRadPerS` reads 1.865230679512024 against 1.865230706178349 computed from the profile; `SpeedForwardNorm` matches an independently computed 0.46870 to float precision when the body and the agent are read in the same call, so no decay can hide a scale error; and probe #5's implied 32.94 m radius falls inside the 17.05 to 48.76 m curvature of the section the car was on
  - The full-lock runs had to move to `FlatGround`, and the instruction to attempt them on a track was wrong: minimum turning radius is 5.361 m, so a full-lock circle is 10.7 m across against a 6.0 m track. The driver reported hitting a wall, which was the correct outcome. `CarAgent`, `ObservationDebug` and `ObservationProbe` were added to the `FlatGround` car, where all thirteen rays read miss and give a third reading of the miss encoding on open ground
...
- [X] T065 Remove `com.unity.ai.assistant` from `unity/SelfDrivingSim/Packages/manifest.json` before the v1.0 submission tag, or write a justification for shipping a pre-release dependency. Constitution VI's clean-clone rule exists because a pre-release version string can be withdrawn or changed under the same number
  ...
  - **Reversed on 2026-08-09. The package was restored.** Removing it took the Unity MCP bridge with it: `com.unity.ai.assistant` is what the editor side of that bridge ships in, and `~/.unity/relay/relay_win.exe` is only the stdio shim in front of it. Within the hour the 54 Unity tools stopped resolving, and the cause was not obvious because the relay process stayed alive and the MCP client still reported the server as connected. It advertised nothing, because there was nothing behind it
  - **The check that passed was the wrong check.** "No script references it, no asmdef lists it" rules out a compile-time dependency and nothing else. An editor service has no `using` line to grep for: it is loaded because it is in the manifest, and it is used through the editor rather than through the code. Written up as research C18, because the same reasoning would remove any editor-only package
  - What survives from the original argument is the real risk, now recorded rather than acted on: `2.17.0-pre.1` is a pre-release string and can be withdrawn or republished under the same number, which would break a clean clone. That is a genuine conflict with Constitution VI and the honest resolution is to state it. **If a clean clone fails to resolve this package, the project still builds and runs; only the MCP tooling is lost.** No script, scene or test depends on it, which is exactly what the original check established and is worth keeping for that reason
...
- [X] T069 Confirm the M2 gate: the scene, vehicle, agent scaffolding and markers exist, the vehicle is keyboard drivable, and the observations have been verified. The blunt version from WORKFLOW section 5 is no keyboard lap, no training

  **The gate is met on 2026-08-09.** Every condition was re-checked rather than taken from the task ticks above, because a tick records what was true when it was written.

  | Gate condition | Evidence | State |
  |---|---|---|
  | Keyboard drivable on the flat plane, limits checked against the dataset | T022 to T025 | met, with one documented exception below |
  | Instability trigger written down and not fired | `StabilityMonitor`, live `ConsecutiveBadRuns` 0 and `ShouldSwitchToKinematic` false | met |
  | Tracks generate reproducibly, geometric checks pass on 100 percent of accepted tracks | T068, all 44 tracks, determinism block printed `REPRODUCIBLE` | met |
  | Training and evaluation seed sets disjoint and recorded | `results/tracks/seed_split.json`: 34 train, 10 eval, **overlap 0** | met |
  | A full lap on at least five different accepted seeds | T051, seeds 37, 29, 1003, 1, 1004, every one `PASS` | met |
  | Every observation read live and checked against a known answer | T062, all nineteen | met |
  | `pytest` green | 237 passed at T068; no `.py` or `pytest.ini` file has changed since that run, confirmed by diff, so it is not re-run for the sake of it | met |
  | Unity EditMode tests green | T067 | met |

  - **The one exception, stated rather than smoothed over: `speed max/P99` does not pass and will not.** Measured 1.0001 to 1.0074 across six laps against a required [1.130, 1.381]. The cause is C9 and C17: these tracks have no straights, so the car is corner-limited for the whole lap and its speed never peaks. It is a real difference between the dataset's course and this generator's, it is recorded in three places, and closing it would mean changing the track generator rather than the gate. Five of the six dataset checks pass
  - The breaches sitting in `stability_log.csv` are runs under the 60 second minimum, so they never qualified toward the three consecutive runs the trigger needs. The trigger is written down, it is instrumented, and it has not fired
  - Two rows had to be removed from that log first: my own editor-driven teleports registered as 21.7 m of idle drift and a fall reset. They were an artefact of moving the car through the editor rather than instability, and leaving them in would have corrupted the record this gate reads
  - **M2 is closed. M3 may begin.** Carried forward: `YawRateNorm` saturates during a spin (T062), and the distribution comparisons must lean on execution metrics rather than marginal histograms (C9, DESIGN 7)
