...
- [~] T062 [US3] Read every observation live during a human drive and check each against a situation whose correct answer is visible. Record which observation was checked against which situation (FR-029, the M2 gate)
  - **Thirteen of nineteen observations verified on 2026-08-09, seed 1004. Recorded in `t062-observation-check.md`.** The remaining six are verified at zero and need one drive to exercise them away from it: the two speeds, the yaw rate and the steering, which only move under throttle, lock or a slide
