...
```jsonc
{
  ...
  // There is deliberately NO timestamp field. See "No generation time" below.
  ...
  // What SC-010 is actually judged on. See "The bound, not the match" below.
  "demand_bound": {
    "within_bound": true,
    "max_required": 0.706566,
    "reference_max": 1.0,
    "exceedance_fraction": 0.0,
    "percentile_gaps": { "p50": -0.001059, "p75": -0.068611, "p95": -0.234494, "p99": -0.222738 },
    "note": "SC-010 is a bound, not a distribution match. ..."
  },

  // Retained beside demand_bound as a diagnostic. It does not decide acceptance.
  "match_report": {
    "scope": "seed 7",
    "distance": 0.041,
    "threshold": 0.05,
...
- **No generation time.** There is no `generated_utc` and there must not be. A timestamp changes
  on every run, and byte-identical output across runs is exactly what SC-007 requires: it is what
  lets a committed track be reviewed in a diff and regenerated by anyone. Generation time moved to
  the batch report, where reproducibility is not claimed.
- **The bound, not the match, decides acceptance.** SC-010 was revised from a distribution match to
  a **bound** on 2026-07-31, because required steering is the geometric minimum needed to follow a
  centre line while the dataset column is steering a human actually applied, corrections and
  overshoot included. The human distribution therefore lies above the geometric one by
  construction, and no track this generator can produce will ever match it. `demand_bound` is what
  the criterion is judged on; `match_report` is retained beside it as a diagnostic and decides
  nothing.
...
- **`match_report.scales` carries the three measured reference distances** from research C15, so a
  ...
  is only meaningful next to the 0.0231 floor and the 0.1142 structureless baseline.
